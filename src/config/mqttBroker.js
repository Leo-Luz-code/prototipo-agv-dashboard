...
server.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    ...
    console.error(
      `   3. Uma instância anterior não foi encerrada corretamente`
    );
    console.error(`\n🔧 Soluções:`);
    console.error(
      `   • Windows: Abra o Gerenciador de Tarefas e encerre processos "node.exe"`
    );
...
broker.on("subscribe", (subscriptions, client) => {
  if (client) {
    subscriptions.forEach((sub) => {
      console.log(
        `[BROKER MQTT] 📬 ${client.id} inscreveu-se em "${sub.topic}"`
      );
    });
  }
});
  ...
  if (client) {
    ...
    if (topic === "agv/rfid") {
      try {
        ...
        console.log(
          `[BROKER MQTT] ✅ Tag "${data.tag}" transmitida para dashboard!`
        );
      } catch (e) {
    ...
    if (topic === "agv/distance") {
      try {
        ...
        console.log(
          `[BROKER MQTT]   Convertidos - Esq: ${esquerda} | Centro: ${centro} | Dir: ${direita}`
        );
        ...
        console.log(
          `[BROKER MQTT] 📡 Enviando para frontend:`,
        ...
        console.log(
          `[BROKER MQTT] ✅ Dados de distância transmitidos via Socket.IO!`
        );
      } catch (e) {
